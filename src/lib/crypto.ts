...
function set25519(r: FE, a: FE): void {
  var i;
  for (i = 0; i < 16; i++) r[i] = a[i] | 0;
}

function car25519(o: FE): void {
  var i,
    v,
    c = 1;
  for (i = 0; i < 16; i++) {
...
function sel25519(p: FE, q: FE, b: number): void {
  var t,
    c = ~(b - 1);
  for (var i = 0; i < 16; i++) {
    t = c & (p[i] ^ q[i]);
...
function pack25519(o: Uint8Array, n: FE): void {
  var i, j, b;
  var m = gf(),
    t = gf();
...
function neq25519(a: FE, b: FE): number {
...
function par25519(a: FE): number {
  var d = new Uint8Array(32);
  pack25519(d, a);
  return d[0] & 1;
}

function unpack25519(o: FE, n: Uint8Array): void {
  var i;
  for (i = 0; i < 16; i++) o[i] = n[2 * i] + (n[2 * i + 1] << 8);
  o[15] &= 0x7fff;
}

function A(o: FE, a: FE, b: FE): void {
  for (var i = 0; i < 16; i++) o[i] = a[i] + b[i];
}

export function Z (o: FE, a: FE, b: FE): void {
  for (var i = 0; i < 16; i++) o[i] = a[i] - b[i];
}

function M(o: FE, a: FE, b: FE): void {
  var v,
    c,
    t0 = 0,
    t1 = 0,
...
function S(o: FE, a: FE): void {
  M(o, a, a);
}

function inv25519(o: FE, i: FE): void {
  var c = gf();
  var a;
...
function pow2523(o: FE, i: FE): void {
  var c = gf();
  var a;
  for (a = 0; a < 16; a++) c[a] = i[a];
  for (a = 250; a >= 0; a--) {
...
export function pack (r: Uint8Array, p: GE): void {
  var tx = gf(),
    ty = gf(),
    zi = gf();
  inv25519(zi, p[2]);
...
function scalarmult(p: GE, q: GE, s: Uint8Array): void {
  var b, i;
  set25519(p[0], gf0);
  set25519(p[1], gf1);
  set25519(p[2], gf1);
...
export function unpackneg (r: GE, p: Uint8Array): 0 | -1 {
  var t = gf(),
    chk = gf(),
    num = gf(),
    den = gf(),
